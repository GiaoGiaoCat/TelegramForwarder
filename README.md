...
- 🧹 **自动清理**：自动清理临时文件，防止硬盘占用
- 📜 **历史转发**：支持按数量或日期范围转发历史消息
...
系统会自动清理 `temp/` 目录中的临时文件，防止硬盘空间被视频等大文件占满。
...
# 文件保留时长（秒），超过此时间的文件将被删除，默认 3600（1小时）
...
**清理日志示例**：
```
INFO - 开始清理临时文件，目录: /path/to/temp
INFO - 已删除文件: video_123.mp4 (大小: 15.34 MB, 存在时间: 1.2小时)
INFO - 清理完成 - 总文件数: 10 (50.23 MB), 清理文件数: 5 (25.67 MB)
```

**详细文档**：查看 [CLEANUP_SCHEDULER_GUIDE.md](CLEANUP_SCHEDULER_GUIDE.md) 了解更多配置和故障排查信息。
...
支持按数量或日期范围转发历史消息到目标聊天，自动应用所有已配置的转发规则。
...
# 使用规则ID 123 转发最近50条消息
/forward_history 50 123

# 转发2024年1月的所有消息
/forward_history 2024-01-01 2024-01-31

# 转发指定日期范围的消息（使用规则123）
/forward_history 2024-01-01 2024-01-31 123

# 转发某一天的消息
/forward_history 2024-10-20 2024-10-20

# 简写形式
/fh 2024-12-01 2024-12-25
```

**功能特点**：
- ✅ 支持按数量转发（1-10000 条）
- ✅ 支持按日期范围转发（YYYY-MM-DD 格式）
- ✅ 自动应用关键字过滤、替换、AI处理等所有规则
- ✅ 实时进度反馈（每10条更新一次）
- ✅ 完整的错误处理和日志记录
- ✅ 消息按时间顺序转发（从旧到新）
...
**注意事项**：
- 日期格式必须为 `YYYY-MM-DD`
- 截止日期会自动包含当天的全部消息（23:59:59）
- 如果规则启用了延迟处理，大量消息转发会比较耗时，建议临时关闭延迟
- 日期范围过大可能匹配大量消息，建议分批处理
